        ...
        <motion.section initial="hidden" animate="visible" variants={containerVariants} className="pt-32 pb-16 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
          <div className="flex-1 space-y-6 w-full">
            ...
            <motion.div variants={textVariants} className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start pt-4">
              <button onClick={() => setIsContactOpen(true)} className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-semibold hover:bg-blue-700 transition-all text-center shadow-lg shadow-blue-500/20 active:scale-95">Hubungi Saya</button>
              <a href="/cv.pdf" className="border border-gray-200 dark:border-white/10 px-8 py-3.5 rounded-2xl font-semibold hover:bg-gray-50 dark:hover:bg-white/10 transition-all text-center active:scale-95 inline-block">Unduh CV</a>
            </motion.div>
          </div>
          ...
          <div className="md:col-span-2">
            <Card className="h-full border-none text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                <div className="bg-white dark:bg-black p-6 rounded-3xl border border-gray-100 dark:border-neutral-800 transition-all hover:-translate-y-1 hover:shadow-md active:scale-[0.98] cursor-default"><p className="font-bold text-blue-600 mb-2">🎮 Game Dev & Modding</p><p className="text-gray-600 dark:text-gray-400">Scripting Lua di Roblox Studio dan desain map 3D Unity untuk Zepeto.</p></div>
                <div className="bg-white dark:bg-black p-6 rounded-3xl border border-gray-100 dark:border-neutral-800 transition-all hover:-translate-y-1 hover:shadow-md active:scale-[0.98] cursor-default"><p className="font-bold text-blue-600 mb-2">🎨 3D AI Rendering</p><p className="text-gray-600 dark:text-gray-400">Render avatar hiper-realistis gaya Blender menggunakan AI generatif.</p></div>
                <div className="bg-white dark:bg-black p-6 rounded-3xl border border-gray-100 dark:border-neutral-800 sm:col-span-2 text-left transition-all hover:-translate-y-1 hover:shadow-md active:scale-[0.98] cursor-default"><p className="font-bold text-blue-600 mb-2">📈 Analisis Pasar</p><p className="text-gray-600 dark:text-gray-400">Investasi rutin aset emas dan pemantauan saham NASDAQ-100 (QQQ) & NVIDIA.</p></div>
              </div>
            </Card>
          </div>
        </motion.div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="fixed inset-0 flex items-center justify-center z-[111] pointer-events-none p-6">
              <div className="bg-white dark:bg-[#121212] w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl pointer-events-auto border border-gray-100 dark:border-white/10 p-8 text-left">
                <div className="flex justify-between items-center mb-8 text-black dark:text-white"><h2 className="text-2xl font-bold tracking-tighter">Hubungi Saya</h2><button onClick={() => setIsContactOpen(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-transform active:scale-90"><X size={20} /></button></div>
                <div className="space-y-4">
                  {socialLinks.map((link) => (
                    <a key={link.name} href={link.href} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded-[1.8rem] border border-gray-100 dark:border-white/5 transition-all group hover:bg-blue-50 dark:hover:bg-blue-600/10 hover:border-blue-600/30 text-black dark:text-white active:scale-95">
                      ...
                      <div className="flex-1"><p className="text-lg font-bold tracking-tight group-hover:text-blue-600 transition-colors">{link.name}</p></div>
